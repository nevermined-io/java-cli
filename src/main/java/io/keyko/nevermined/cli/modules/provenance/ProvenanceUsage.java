...
@CommandLine.Command(
        name = "usage",
        description = "Provenance registration of the usage of an asset")
public class ProvenanceUsage implements Callable {
    ...
    @CommandLine.Option(names = { "-s", "--signature" }, defaultValue = "",
            description = "Signature of the provenance id given by the agent.")
    String signature;
    ...
    CommandResult provenanceMethod() throws CLIException {
        ...
        try {
            ...
            command.printHeader("Registering Usage Provenance Entry:");
            command.printSubHeader(did);

            command.cli.progressBar.start();

            final boolean result = command.cli.getNeverminedAPI().getProvenanceAPI()
                    .used(provenanceId, new DID(did), agentId, activityId, signature, attributes);
